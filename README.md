...
- The --last song flag requires first reading the last song played from the mpd log and thus logging must be enabled via mpd's configuration file. `mpdtags` uses `/var/log/mpd/mpd.log` as the default location; alternate locations may be specified by using either `mpdtags --last=/path/to/mpd.log` or using the `MPD_LOG=/path/to/mpd.log` variable either on the CLI or as an environmental variable, e.g.:
   - `$ MPD_LOG=/parth/to/mpd.log mpdtags --last`
   - `$ export MPD_LOG=/path/to/mpd.log; mpdtags --last`
  NB: --last will only work if `mpdtags` has read access to the log (and will not work for remote connections).
